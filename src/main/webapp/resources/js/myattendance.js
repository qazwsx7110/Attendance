		...
		xhr.setRequestHeader(header, token);
		 ...
		 async: false,
           success : function(data) {
			...
			console.log(value);
			let cdate = new Date(value.attDate);
		
			console.log("날짜데이터일수:"+cdate.getDate());
			let cstarttime = new Date(value.startTime);
			let cendtime;
			
				if(((value.state=="지각") || (value.state=="정상출근"))&&(value.endTime!=null)){
				cendtime=new Date(value.endTime);
				$('#'+ cdate.getDate()).html("출근 "+cstarttime.getHours()+":"+cstarttime.getMinutes()+"<br>퇴근 "+cendtime.getHours()+":"+cendtime.getMinutes()+"<br>"+value.state);
				}
				if(((value.state=="지각") || (value.state=="정상출근"))&&(value.endTime==null)){
			
				$('#'+ cdate.getDate()).html("출근 "+cstarttime.getHours()+":"+cstarttime.getMinutes()+"<br>"+"<br>"+"<br>");
				}
				if(value.state=="휴가"){
				...
				let cdategetdate;
				if(cdate.getDate()<10){
				cdategetdate="0"+cdate.getDate();
				}
				else{
				cdategetdate=cdate.getDate();
				}
				$('#'+ cdategetdate).html(""+"<br>"+"<br>"+value.state);
				$('#h'+ cdategetdate).css("color","#ed5353");        
		        $('#'+ cdategetdate).parent().css("backgroundColor","#f7f7f7");		      		      
				
				}
				
				if(value.state=="결근"){
					$('#'+ cdate.getDate()).html(""+"<br>"+"<br>"+value.state);
					}
...
document.getElementById("download").addEventListener('click',download);
	...
	function download(){
		...
		console.log(enddate);
          //엑셀 다운로드
           $.ajax({
			url : '/exceldownload/attendancedownload',
           type : 'POST',
            beforeSend : function(xhr){
		xhr.setRequestHeader(header, token);
	     } ,
           data : {
            ...
            success: function(blob) { 
                ...
                link.download = "근태기록:"+ $('#username').val()+".xlsx"; 
